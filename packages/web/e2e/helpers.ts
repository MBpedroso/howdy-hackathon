...
/**
 * The one class of console error the suite tolerates: missing human-supplied art.
 *
 * Two sets of raster files are generated by a human and dropped in — the cast's
 * portraits in `public/agents/*.png` (spec §13 delta 17) and the four mascot faces
 * in `public/fighters/*.png`. Until a file is there the `<img>` 404s — visibly, in
 * the console — and `src/ui/portrait.ts` falls back to the SVG placeholder it
 * already rendered underneath. That is the intended state, and it is deliberately
 * *not* hidden from the browser: the day the PNGs land this predicate stops
 * matching anything and nothing else about the suite changes.
 *
 * Narrow on purpose — it matches only a failed load of one of those eight files, so
...
export function isMissingPortrait(text: string): boolean {
  const art = /agents\/(analyst|coder|judge|boss)\.png|fighters\/(neptune|saturn|jupiter|earth)\.png/;
  return art.test(text) && /Failed to load resource|404|ERR_/.test(text);
}
