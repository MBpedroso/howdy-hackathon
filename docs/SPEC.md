...
| 26 | §6.3: the loop is Analyst → Coder → gates, "max 4 attempts, then fallback pool"; every candidate file is written by the Coder | The Judge gained one deterministic step of its own, **calibration** (`agents/src/calibrate.ts`): when a candidate fails Gate 3 on **FAIR too hard only** (ACTIVE clean, ADAPTED not blocking — read structurally from `detail`, never the reason string), the loop appends a marked block to the file that renames the Coder's `init`/`decide` and wraps them with a `THROTTLE` in `[0.1, 1]`: after every attack the boss is held for `round((1/THROTTLE − 1) × 45)` ticks and marches straight legs meanwhile, so ACTIVE still holds. It then re-runs **all four gates** per value — bracket ×0.5, then bisect at the geometric midpoint, at most 6 steps, ~1 s each — and ships the first value that passes; too-easy candidates are left to the Coder retry. Events `calibrate.step` / `calibrate.done`; the shipped `source` is the throttled file and its diff is what the player sees. Downward-only and monotone by construction — measured on the round-2 fallback with the beat and spawn halved: 0.75 → 0.41 → 0.00 at 1.0 / 0.5 / 0.25 | Three real rounds on 2026-09-11 (claude-cli) showed the Coder cannot aim a number: for band 0.35–0.50 it wrote 0.56, 0.00, 0.84, 0.57, 0.65 — every file a plausible counter, every file too hot or too cold — and with the Analyst at 14–27 s and a Coder call at 20–37 s only two attempts fit 90 s. The gates cost ~1 s, so the numeric aiming belongs to the harness, exactly as the humans tuned the fallback pool (`fallback/round2/metronome.js`: "700 measures 0.46, 1600 measures 0.36"). A first version asked the *Coder* to declare a `PRESSURE` knob; live it measured 0.86 → 0.90 → 0.84 across 1.0 / 0.5 / 0.25 because the model scaled slam odds while the kills came from burst and minions ([`evidence/local-live-2026-09-11-coder-knob-flat.json`](evidence/local-live-2026-09-11-coder-knob-flat.json)). An LLM-wired knob is not monotone; a harness-owned one is. The learning stays the Coder's — the throttle changes how *often* the same read lands, not what is read. First live results: round 2 approved in 44 s (0.78 → 0.49 at 0.5), round 3 in 57 s (0.77 → 0.60 at 0.771, ADAPTED met) |
...
7. The Round 2 boss beats the `Mimic` bot ≥ 70% in the sim for that run. *(In round 2: measured and reported on every run, not enforced — see §13 delta 23 for the measurement that changed this. From round 3 the same assertion **does** enforce, at 0.75 / 0.85 / 0.95 — §13 delta 24.)*
...
| 24 | §6.2: one fairness band per round, escalating 0.35–0.50 / 0.45–0.60 / 0.50–0.65 / 0.55–0.70, and (with delta 23) ADAPTED reported but never enforced | **Two escalations, not one.** FAIR's bands move to 0.50–0.65 / 0.60–0.75 / 0.65–0.95 for rounds 3, 4 and 5 (**round 2 is byte-for-byte unchanged**), and **ADAPTED blocks from round 3** at 0.75 / 0.85 / 0.95 — either the absolute number or the existing relative route (incumbent's rate against the identical Mimic + `ADAPTED_MARGIN`) satisfies it. Round 2 keeps delta 23 exactly: 0.70, advisory, `blocking: false`. `ADAPTED_MIN` and `ADAPTED_BLOCKS` are per-round tables with `adaptedBlocks(round)` beside them (`harness/src/gates/balanceConfig.ts`), and a blocked ADAPTED produces a rejection sentence as concrete as FAIR's, because the Coder gets it verbatim on the retry: `0.62 vs Mimic — didn't adapt (need >= 0.85 for round 4, or >= 0.81 = incumbent 0.71 + 0.10)` | The human's requirement for rounds 3→5: harder, *but* **"the boss must really learn from the player's movement, not spam"**. Those are two different dials and only one of them was moving. Raising FAIR alone buys difficulty against four scripted bots that never change — more projectiles, more area denial, a boss that wins by volume; raising ADAPTED buys it against a bot rebuilt from **this player's** replay, which firing more cannot beat. So the band rises slowly and stays the cap on spam, and the assertion that means "it learned" is the one that escalates and refuses. Delta 23's argument is untouched where it was measured: it showed ADAPTED unsatisfiable **in round 2**, where FAIR capped the boss at 0.50 while the incumbent scored 0.710 against a Mimic of a flawless human run — so round 2 is exactly where it stays advisory. From round 3 the ceiling is 0.65–0.95, which is the room that argument said did not exist, and the relative route is always open. Evidence: the six fallbacks were re-tuned to the new bands and every one passes Gates 1–4 (`packages/server/test/fallback.test.ts`, panel rates in each file's own header — e.g. `crossfire` 0.61 → 0.73 by aiming its cone at the intercept rather than at the player, `bellringer` 0.54 → 0.70 by pairing the ring with the slam inside the dash's 45-tick cooldown); round 2's recorded evidence re-grades **identically** (`pnpm --filter @rematch/harness recheck:adapted` and `regrade:live`, unchanged counts). There is **no recorded eval for rounds 3–5** — every committed run is a round-2 run — so the three thresholds are argued from the round-2 corpus's distribution (incumbent 0.54–1.00 against its Mimics, mean 0.804) and not calibrated against a live run |
| 25 | AC 5: the interlude finishes in **45 s** or falls back visibly in 50 s — one number, hardcoded in the client | 45 s is the **default and the deployed behaviour**, unchanged. On the one row where the client probes a server it can see — local host, no `?agent=`, no `VITE_API_BASE` — the boot probe now reads `deadlineMs` off `/api/health` and the round adopts it **when it is larger**, capped at 180 s (`adoptDeadlineMs`, `web/src/interlude/index.ts`). Precedence: an explicit `?deadline=<ms>` beats everything, then the advertised number, then 45 s; an advertised value at or below 45 s is ignored. The adopted number is also what rides the request as `budgetMs` — the two are the same number by construction — and the header clock's `over` state keys off it, so a 90 s round does not turn red at 45 | Delta 21 made the client's budget the bound on the server's loop, which was right, and then the bound became the ceiling. With `REMATCH_PROVIDER=claude-cli` the local server is configured for 90 s and `/api/health` says so, but the browser sent `budgetMs: 45000`, so `clampToClientBudget` cut the loop to 43 s — and a CLI turn costs ~14 s of Analyst plus ~26 s per Coder attempt, so 43 s buys **one** attempt and the fallback is certain before the run starts. Three runs on 2026-09-11 measured exactly that, each with `request.budgetMs 45000`, `summary.attempts 1`, `summary.ms` 43 007 / 43 006 / 43 003 and `reason: 'deadline'`: `artifacts/server/rewrite-2026-09-11T16-18-39-211Z.json`, `…T16-20-25-072Z.json`, `…T16-21-33-548Z.json`. The same machine at a 120 s budget approved a boss in 82 957 ms and again in 72 253 ms (`…T16-23-40-761Z.json`, `…T16-25-23-475Z.json`), which is the whole difference between the product's thesis working locally and never being seen. The deployed site is recorded-only and has no probe, so AC 5's 45 s is still the promise everywhere it is made to a player; what changed is that a developer's own server no longer has to lie about how long it has |
